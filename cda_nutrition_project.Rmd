...
```{r}
df_modified_bloodpressure_demography|>
  dplyr::select(c("nh_black_white","SEQN","blood_pressure","clg_education","RIAGENDR","DMDYRUSR","RIDAGEYR"))|>
...
model_reg1 <- glm(
  ...
  +clg_education+ RIAGENDR+ DMDYRUSR+RIDAGEYR)^2
...
table(nh_bp_edu_gender[,"blood_pressure"])
nh_bp_gender_table
...
```{r}
mod_nh_bp_meal=glm(as.factor(blood_pressure)~(nh_black_white + clg_education
  +clg_education+ RIAGENDR+ DMDYRUSR+RIDAGEYR+self_meal_prepare
)^2,
     family = binomial(link = "logit"), 
  
  data =df_nh_bp_edu_gender_immigration_self_meal_prepare|>filter(nh_black_white!=0) 
)
```
...
df_modified_bloodpressure|>
  ...
  dplyr::select(c("SEQN","total_activity_index","blood_pressure"))
